...
export default function CoinsModal({
  ...
  const handleChange = (
    ...
    if (type == "add") setAddValues((prev) => ({ ...prev, [token]: value }));
    else {
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="font-bold text-primary-content font-fantasy text-outline-secondary text-2xl underline">
          Edit Coins
        </h3>
        ...
        <form onSubmit={handleSubmit} className="w-full my-5">
          <div className="flex flex-row items-center ring-2 ring-success ring-inset bg-secondary-content/60 gap-4 py-2 mb-5 rounded">
            <p className="text-success font-fantasy font-bold text-outline-primary-content text-center w-1/6 text-xl self-center">
              Add:
            </p>
            <div className="flex gap-4 w-5/6 mx-2">
              {coins.map((coin) => (
                <label key={coin.name} className="floating-label">
                  ...
                  <input
                    type="number"
                    value={addValues[coin.token] || 0}
                    onChange={(e) => handleChange(e, "add", coin.token)}
          ...
          <div className="flex flex-row items-center ring-2 ring-error ring-inset bg-secondary-content/60 gap-4 py-2 mb-5 rounded">
            <p className="text-error font-fantasy font-bold text-outline-primary-content text-center w-1/6  text-xl self-center">
              Subtract:
            </p>
            <div className="flex gap-4 w-5/6 mx-2">
              {coins.map((coin) => (
                <label key={coin.name} className="floating-label">
                  ...
                  <input
                    type="number"
                    value={subtractValues[coin.token] || 0}
                    onChange={(e) => handleChange(e, "subtract", coin.token)}
                    placeholder={coin.token}
                    className="input input-md"
                  />
                </label>
